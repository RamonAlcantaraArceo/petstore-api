...
from petstore_core.repositories.base import PetRepository
from petstore_core.schemas.pet import Pet, PetCreate, PetUpdate
...
class PetService:
    ...
    async def add_pet(self, pet: PetCreate) -> Pet:
        ...
        Raises:
            ValidationError: If pet name is empty.
    ...
    async def update_pet(self, pet: PetUpdate) -> Pet:
        ...
        try:
            ...
            await self._rollback()
            raise NotFoundError("Pet not found") from exc
        except Exception:
            await self._rollback()
            raise
